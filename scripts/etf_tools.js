...
// ── Live price cache (fetched from data/etf_prices/) ──────────────
// Structure: { "VAS.AX": { prices:[{date,close},...], returns:{2020:12.3,...} }, ... }
const priceCache = {};
let priceIndex   = null;   // list of available tickers from index.json

async function loadPriceIndex() {
  if (priceIndex) return priceIndex;
  try {
    const r = await fetch('/data/etf_prices/index.json');
    if (!r.ok) { priceIndex = []; return []; }
    const d = await r.json();
    priceIndex = d.tickers || [];
    return priceIndex;
  } catch { priceIndex = []; return []; }
}

async function loadTickerPrices(yahooTicker) {
  // yahooTicker = e.g. "VAS.AX" or "VAS" (we normalise)
  const t = yahooTicker.endsWith('.AX') ? yahooTicker : yahooTicker + '.AX';
  if (priceCache[t]) return priceCache[t];
  try {
    const r = await fetch(`/data/etf_prices/${t}.json`);
    if (!r.ok) return null;
    const d = await r.json();
    // Pre-compute annual returns from price series
    const returns = {};
    const prices  = d.prices || [];
    // Group by year, take first price of each year, compare year-on-year
    const byYear  = {};
    prices.forEach(p => { const yr = p.date.slice(0,4); if(!byYear[yr]) byYear[yr]=p.close; });
    const yrs = Object.keys(byYear).map(Number).sort((a,b)=>a-b);
    for(let i=1;i<yrs.length;i++){
      const ret = (byYear[yrs[i]] / byYear[yrs[i-1]] - 1) * 100;
      returns[yrs[i-1]] = parseFloat(ret.toFixed(2));
    }
    // CAGR
    const first = prices[0]?.close, last = prices[prices.length-1]?.close;
    const years  = prices.length > 0
      ? (new Date(prices[prices.length-1].date) - new Date(prices[0].date)) / (1000*60*60*24*365.25)
      : 0;
    const cagr = (first && last && years > 1) ? ((Math.pow(last/first, 1/years) - 1)*100) : null;
    priceCache[t] = { prices, returns, cagr };
    return priceCache[t];
  } catch { return null; }
}

// Returns historical annual returns for a ticker, merging live data over stored data
function getHistoricalReturns(ticker) {
  // Check live cache first (most accurate)
  const t = ticker.endsWith('.AX') ? ticker : ticker + '.AX';
  if (priceCache[t]?.returns && Object.keys(priceCache[t].returns).length > 0)
    return priceCache[t].returns;
  // Fall back to stored etf_data.json returns
  return etfData?.etfs[ticker]?.historicalReturns || {};
}
...
let compState = {
  ...
  liveTickerInput: '',   // raw user input for any ASX/global ticker not in the ETF list
  initial: 10000, monthly: 500,
  // projection
  projStartDay: 1, projStartMonth: NOW.getMonth(), projStartYear: CY, years: 20,
  // historical (day-exact)
...
function getETF(ticker) {
  ...
  if (ticker==='LIVE') {
    // Live ticker from priceCache — ticker symbol is in compState.liveTickerInput
    const t = compState.liveTickerInput.trim().toUpperCase();
    const key = t.endsWith('.AX') ? t : t + '.AX';
    const cached = priceCache[key];
    return {
      name: t, issuer: 'Live (Yahoo Finance)',
      annualReturn: cached?.cagr ?? 10.0,
      mer: 0.0, dividendYield: 0.0, inceptionYear: 2000,
      historicalReturns: cached?.returns ?? {},
...
function calcMonthly() {
  ...
  for (let i = 0; i < months.length; i++) {
    ...
    // ── MER treatment ──────────────────────────────────────────────
    // Historical returns come from Yahoo Finance adjusted-close prices or fund
    // factsheets. BOTH sources already reflect MER — the fund deducts it daily
    // from NAV, so the year-on-year price change is already net-of-fees.
    // Deducting MER again would double-count it and understate returns.
    // Projection mode uses a gross annualReturn estimate, so MER IS deducted there.
    const merDeduction = mode === 'historical' ? 0 : (etf.mer / 100);

    // When DRP is off, dividends are paid out as cash rather than reinvested.
    // The stored total return includes dividend reinvestment, so subtracting the
    // dividend yield approximates the capital-only return component.
    ...
    data.push({
      ...
      isDivPayment,
      ts: mo.ts   // ← used for interpolated daily date in hover
    });
  }
...
function buildChart(data, drip, svgId, tipId) {
  ...
  // Only labeled points get x-axis ticks; thin out to max ~9
...
function tickerOpts(sel, custom=false) {
  const G = {
    ...
    'International':['VGS','BGBL','IVV','NDQ','CNDX','QUAL','MOAT','VESG',
...
function renderCompounding() {
  const { mode, ticker, initial, monthly,
    projStartDay, projStartMonth, projStartYear, years,
    histStartDay, histStartMonth, histStartYear,
    histEndDay,   histEndMonth,   histEndYear, drip, custom, liveTickerInput } = compState;
  const etf = getETF(ticker);
  ...
  const liveKey = liveTicker.endsWith('.AX') ? liveTicker : liveTicker + '.AX';
  const liveLoaded = ticker==='LIVE' && priceCache[liveKey];
  const livePending = ticker==='LIVE' && !priceCache[liveKey] && liveTicker;
  ...
  ${mode==='historical'&&!hasHist?`<div style="font-size:.75rem;color:#f59e0b;background:rgba(245,158,11,.08);border:1px solid rgba(245,158,11,.2);border-radius:6px;padding:.5rem .8rem;margin-bottom:1rem;">
    ⚠ No year-by-year data for <strong>${ticker==='LIVE'?liveTicker:ticker}</strong>. ${ticker==='LIVE'&&!liveLoaded&&liveTicker?'Price file not yet fetched by daily update — run the GitHub Action to populate data/etf_prices/.':'All periods use the average return ('+etf.annualReturn+'%).'}
  </div>`:''}
    ...
    <div class="etf-info-box">
      ...
      <div style="font-size:.8rem;color:var(--text-secondary);margin-top:.25rem;">
        ${ticker==='LIVE'&&liveLoaded
          ? `CAGR: <span style="color:var(--fitc-green);">${etf.annualReturn?.toFixed(1) ?? '—'}%</span> &nbsp;·&nbsp; MER: <span style="color:#ef4444;">not in dataset</span> &nbsp;·&nbsp; <span style="color:#f59e0b;">${Object.keys(histRets).length} years of data</span>`
  ...
  <!-- Summary cards -->
  <div style="display:grid;grid-template-columns:repeat(${drip?3:4},1fr);gap:1rem;margin-bottom:1.2rem;">
  ...
  <div style="display:flex;gap:1.5rem;justify-content:center;flex-wrap:wrap;margin-top:.5rem;font-size:.75rem;color:var(--text-secondary);">
    <span><span style="color:#4ade80;">——</span> Portfolio Value</span>
...
function attachListeners() {
  ...
  if(currentTab==='compound') {
    ...
    document.getElementById('live-ticker-load')?.addEventListener('click', async () => {
      ...
      const loaded = await loadTickerPrices(raw);
      if(btn){ btn.textContent='Load ↗'; btn.disabled=false; }
      if(loaded){
        compState.ticker='LIVE';
        compState.mode='historical';
        // Auto-set historical date range to match available data
        const prices = loaded.prices;
        if(prices.length > 0){
          const first = new Date(prices[0].date);
          const last  = new Date(prices[prices.length-1].date);
          compState.histStartDay=1; compState.histStartMonth=first.getMonth(); compState.histStartYear=first.getFullYear();
          compState.histEndDay=last.getDate(); compState.histEndMonth=last.getMonth(); compState.histEndYear=last.getFullYear();
        }
        rerender();
      } else {
        alert(`Could not load data for "${raw}". Check that:\n1. The ticker exists on Yahoo Finance (try "${raw}.AX" for ASX stocks)\n2. The daily GitHub Action has run and populated data/etf_prices/\n\nAlternatively use ✏ Custom mode to enter manual return assumptions.`);
      }
    });
...
_css.textContent=`
  ...
  @media(max-width:640px){ .etf-stat-value{font-size:1rem;} }
`;
